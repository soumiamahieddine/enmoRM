    ...
    extends abstractBusinessReplyMessage
{
    ...
    public function generate($message)
    {
        ...
        $this->setTransferDate($message->operationDate);
    ...
    protected function setTransferDate($date)
    {
        $dateText = $this->message->xml->createTextNode($date->format('Y-m-d\TH:i:s'));

        if (!$dateElement = $this->message->xPath->query("medona:TransferDate")->item(0)) {
            $dateElement = $this->message->xml->createElement('TransferDate');
