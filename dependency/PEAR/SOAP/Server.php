    ...
    function &callMethod($methodname, &$args)
    {
        ...
        if ($args) {
            ...
            if (isset($this->soapobject) && is_object($this->soapobject)) {
                $ret = call_user_func_array(array(&$this->soapobject, $this->methodname), $args);
            } else {
        ...
        if (isset($this->dispatch_objects[$namespace])) {
            ...
            for ($i = 0; $i < $c; $i++) {
                ...
                if (method_exists($obj, '__dispatch')) {
                    if ($map = $obj->__dispatch($methodname)) {
                        $this->method_namespace = $map['namespace'];
                        $this->methodname = $map['method'];
                        $this->soapobject =& $map['controller'];
                        return true;
                    }
